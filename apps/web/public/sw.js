...
// Only cache actual static files (not dynamic Next.js routes)
const STATIC_ASSETS = [
  '/manifest.json',
  '/offline.html',
];
...
self.addEventListener('fetch', (event) => {
  ...
  // BUT never intercept auth routes — they must always go to network
  if (request.method !== 'GET') {
    if (NETWORK_ONLY_ROUTES.some(route => url.pathname.startsWith(route))) {
      return; // Let auth/sync requests pass through to network directly
    }
